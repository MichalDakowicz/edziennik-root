...
Ask targeted PM discovery questions before drafting the ticket, then generate the same output structure as `create-ticket` with the answers embedded into scope, acceptance criteria, and TDD planning.

The finalized ticket must be suitable for immediate creation as a GitHub issue and addition to GitHub Project `https://github.com/users/MichalDakowicz/projects/2`.
The selected Fibonacci story point value must also be used as the project's `Estimate` field value.
...
Use the same sections and order as `create-ticket`:
1. Title
2. Summary
3. User Story
