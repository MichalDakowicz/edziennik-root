# GŁÓWNY SYSTEM ORKIESTRACJI Codex (MODÉA)

Ten plik zarządza głównym przepływem pracy, wyborem agentów i restrykcyjnymi zasadami całego projektu Modéa (e-dziennik). 
Jako główny proces Codex, Twoim pierwszym zadaniem jest zrozumienie żądania użytkownika, wybranie odpowiedniego agenta z tabeli poniżej i zaaplikowanie odpowiedniego rygoru (szczególnie trybu Caveman, obsługi GitHub CLI oraz TDD).
